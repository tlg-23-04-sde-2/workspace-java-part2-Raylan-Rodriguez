...
public class InMemoryCatalog implements Catalog {
    ...
    public Collection<MusicItem> findByKeyword(String keyword) {
        ...
        for (MusicItem item : catalogData) {
            if (item.getArtist().equalsIgnoreCase(keyword) ||
                    item.getTitle().equalsIgnoreCase(keyword)) {
                result.add(item);
            }
        }
        return result;
     ...
     */
    public Collection<MusicItem> selfTittleAlbums() {
        Collection<MusicItem> result = new ArrayList<>();
        for (MusicItem selfTittle : catalogData) {
            String artistName = selfTittle.getArtist(); //local alias -- will only work within this for-each loop
            String title = selfTittle.getTitle();       //local alias  ''''''''''''''''''''''''''''''''''''''''
            if (artistName.equalsIgnoreCase(title)) {
                result.add(selfTittle);
            }
        }
    ...
    public Collection<MusicItem> getItemsAtSpecifiedPrice(double maxPrice) {
        Collection<MusicItem> result = new ArrayList<>();
        for (MusicItem rockItem : catalogData) {
            MusicCategory rock = rockItem.getMusicCategory();   // Local alias
            if (rockItem.getPrice() <= maxPrice && rock.equals(MusicCategory.ROCK)) {
                result.add(rockItem);
            }
        }
    ...
    public int genreCount(MusicCategory genre) {
        int count = 0;
        for (MusicItem record : catalogData) {
            if (record.getMusicCategory().equals(genre)) {
                ++count;
            }
        }
    ...
    public double averagePrice() {
        double priceSum = 0.0;
        for (MusicItem prices : catalogData) {
            priceSum += prices.getPrice();
        }
     ...
     */
    public Collection<MusicItem> cheapestItemInCategory(MusicCategory genre) {
        Collection<MusicItem> cheapest = new ArrayList<>(findByCategory(genre));
        Collection<MusicItem> cheapestAlbum = new ArrayList<>();
        MusicItem cheapestItem = Collections.min(cheapest, Comparator.comparingDouble(MusicItem::getPrice));
        cheapestAlbum.add(cheapestItem);
     ...
     */
    public Double averagePriceOfGenre(MusicCategory genre) {
        Collection<MusicItem> genreList = new ArrayList<>(findByCategory(genre));
        double priceSum = 0.0;
        for(MusicItem album : genreList) {
            priceSum += album.getPrice();
        }
        return priceSum / genreList.size();
    }
    ...
    public Boolean isAllGreaterOrEqualTen() {
        boolean result = true;
        for(MusicItem price : catalogData) {
            if(price.getPrice() < 10) {
                result = false;
            }
        }
        return result;
    ...
    public Boolean doWeSellItem(MusicCategory genre) {
        boolean result = false;
        for(MusicItem album : catalogData) {
            if(album.getMusicCategory().equals(genre)) {
                result = true;
            }
        }
        return result;
    ...
    public List<String> allPopItemsInNaturalOrder() {
        Collection<MusicItem> allPopItems = new ArrayList<>(findByCategory(MusicCategory.POP));
        List<String> tittles = new ArrayList<>();
        for (MusicItem album : allPopItems) {
            tittles.add(album.getTitle());
        }
        tittles.sort(null);
        return tittles;
    }
